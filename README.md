...
稍加计算就会发现，任意的奇数密文字母都有 2 个明文字母与之对应；而任意的偶数密文字母没有明文字母与之对应。
...
> 爱丽丝用自己的私钥对要发送的信息以 RSA 算法进行签名，并将签名和明文一起发送给鲍勃，然后让鲍勃用爱丽丝自己的公钥以 RSA 算法验证签名，比较它是否与明文信息相等就可以了！

这里之所以发送明文，是因为爱丽丝不关心 QQ 会不会读取。如果要发送敏感信息，可以先加密，再签名。
